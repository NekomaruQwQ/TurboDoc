import type { Page, PageBlock, PageBlockNameAction, PageLayout } from "@/core/explorer";
import {
    reorderPinnedPages,
...
export function normalizePageCollections(
    ...
    input: unknown,
    resolvePage: ResolveSourcePage): PageCollections {
 ...
 * Reads and writes are supplied by the source model so every action validates the
...
export function createCollectionLayout(
    ...
    resolvePage: ResolveSourcePage): PageLayout {
    const state = read();
