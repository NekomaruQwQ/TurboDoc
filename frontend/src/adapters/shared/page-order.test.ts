...
/** Resolve the two exact test origins without depending on a source catalog. */
function resolvePage(input: string): ResolvedSourcePage | null {
    let url: URL;
    try {
        url = new URL(input);
    } catch {
    ...
    const sourceId = url.origin === "https://minecraft.wiki"
        ? "minecraft-wiki"
    ...
    identity.hash = "";
    return { sourceId, url: url.href, identity: identity.href };
}
