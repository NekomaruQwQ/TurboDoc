...
export type RustCrateSourceContext = SourceModelContext<RustCrateSourceData>;

/** Flat persisted data owned by the Rust crate source. */
export interface RustCrateSourceData {
    /** Concrete schema version; no shared SourceData base is required. */
    schemaVersion: 1;
    /** User-selected crates keyed by their canonical package names. */
    crates: Record<string, CrateData>;
}

/** Persisted state for one selected Rust crate. */
...
/** Data-only configuration consumed by the Rust crate adapter. */
export interface RustCrateRules {
    /** Canonical topic-selection landing page. */
    readonly homeUrl: string;
    /** Crates created only for a genuinely missing source resource. */
    readonly starterCrates: readonly string[];
}

const crateDataSchema = z.object({
    currentVersion: z.string(),
    pinnedPages: z.array(z.string()),
});

const rustCrateSourceDataSchema = z.object({
    schemaVersion: z.literal(1),
    crates: z.record(z.string().regex(/^[a-z0-9_-]+$/i), crateDataSchema),
});

/** Adapter for docs.rs, standard-library rustdoc, and windows-docs-rs crates. */
export const RustCrateAdapter: Adapter<RustCrateSourceData, RustCrateRules> = {
    resolve(definition): SourceModel<RustCrateSourceData> {
        const home = new URL(definition.rules.homeUrl);
        return {
            id: definition.id,
            name: definition.name,
            homeUrl: definition.rules.homeUrl,
            presentation: {
            ...
            initializeData(raw, exists) {
                if (exists) {
                    return {
                        data: rustCrateSourceDataSchema.parse(raw),
                        persist: false,
                    };
                }
                return {
                    data: {
                        ...
                        crates: Object.fromEntries(
                            definition.rules.starterCrates.map(name => [name, {
                                currentVersion:
            ...
            matchUrl(url) {
                try {
                    ...
                    if (candidate.origin === home.origin &&
                        candidate.pathname === home.pathname)
                        return true;
                } catch {
                    return false;
                }
            ...
            render,
            setupEffects: setupRustCrateEffects,
        };
    },
};

/** Exact definition type accepted by the Rust crate adapter. */
export type RustCrateSourceDefinition =
    SourceDefinition<RustCrateSourceData, RustCrateRules>;

/** Derive the crate source view from current source data and navigation. */
function render(ctx: RustCrateSourceContext): SourceView {
...
function getImportCratesAction(
    ctx: RustCrateSourceContext): ExplorerInputAction {
    return {
        type: "input",
        name: "Import",
