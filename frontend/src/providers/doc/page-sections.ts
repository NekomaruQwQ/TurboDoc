import type { Page, PageBlock, PageLayout } from "@/core/data";

/** Placement in a provider-owned contiguous reading-order span. */
export interface ProviderPagePlacement {
...
/** A site chooses exactly one organization policy. */
export type DocPageOrganization =
    | { type: "user-collections" }
    | {
        type: "provider-sections";
        /** Return null for unknown pages; failures must never hide pins. */
        resolvePagePlacement(url: URL): ProviderPagePlacement | null;
    };
...
export function createSectionLayout(
    pages: readonly Page[],
    resolve: (url: URL) => ProviderPagePlacement | null): PageLayout {
    const loose: Page[] = [];
    const placed: { page: Page; placement: ProviderPagePlacement }[] = [];
    for (const page of pages) {
        if (page.pinned === null) continue;
        let placement: ProviderPagePlacement | null = null;
        try {
