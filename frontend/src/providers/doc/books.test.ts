...
describe("official Rust bookshelf", () => {
    test("all fifteen sources own distinct canonical homes and have complete outlines", () => {
        expect(RUST_BOOK_SITES).toHaveLength(15);
        for (const site of RUST_BOOK_SITES) {
            expect(RUST_BOOK_SITES.filter(candidate => candidate.ownsUrl(new URL(site.homeUrl))).map(owner => owner.id)).toEqual([site.id]);
            const book = BOOK_CATALOG.find(book => book.id === site.id);
            expect(book).toBeDefined();
            for (const alias of book?.aliases ?? []) {
                const url = new URL(alias);
                site.normalizeUrl?.(url);
                expect(url.href).toBe(site.homeUrl);
            }
        }
    ...
    test("book scopes reject lookalikes, sibling paths and Rustdoc", () => {
        ...
        ]) expect(RUST_BOOK_SITES.some(site => site.ownsUrl(new URL(input)))).toBeFalse();
    });

    test("known book preview resolves its full section ancestry", () => {
        const site = RUST_BOOK_SITES.find(site => site.id === "rust-book");
        const organization = site?.organization;
        if (organization?.type !== "provider-sections") throw new Error("Expected book sections");
        expect(organization.resolvePagePlacement(new URL(`${site?.homeUrl}ch03-05-control-flow.html`))?.titlePath)
