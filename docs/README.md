...
```
src/index.tsx (entry point, workspace/cache loading, auto-save)
└── AppContextProvider
...
- **ExplorerProvider** (inline): Constructs `ProviderContext`, calls `provider.render()`, renders provider actions and groups
...
View models contain callbacks (e.g., `setPinned`, `setCurrentVersion`, `invoke`) that update provider data via Immer. View models are derived on every render — never memoized, never serialized. See `core/data.d.ts` for the full type definitions.
...
- Split into `workspace.json` (user data) + `cache.json` (API data)
- Smaller workspace file, independent cache management, non-fatal cache failures
- Persisted via Hono HTTP API (`/api/v1/workspace`, `/api/v1/cache`)
- Auto-save on every state change (no debouncing — files are small)
- Originally planned localStorage for cache, but kept server-side persistence for simplicity
...
- `ProviderContext` constructed inline with update callbacks in `ExplorerProvider`

**Hybrid IPC**
- Workspace/cache CRUD via Hono HTTP API (`hono/client` for type safety)
...
- Cache load errors return `null` (non-fatal); cache save errors log but don't throw
...
│   ├── index.tsx               # React entry point (workspace/cache loading, auto-save, IPC listener)
