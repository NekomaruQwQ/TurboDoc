...
- **Native boundary**: All webview→host application communication uses REST-style `fetch()` under `/api/*`; dev uses the Vite origin while release uses the unmapped `https://api.turbodoc.example` origin. WebView2 intercepts that namespace, answers release preflights, passes `/api/ready` through to Vite only in dev mode, dispatches generic and per-source persistence routes to Rust, and rejects every other path. `src/app/webview_handlers.rs` builds host→webview calls to named functions under `window.__turboDoc__`, while the TurboDoc-agnostic WebView2 wrapper executes their source in FIFO order through `ExecuteScriptWithResult`; there is no WebView2 message protocol or generic event dispatcher.
...
├── src/
│   ├── app/
│   │   ├── mod.rs                # native app façade and shared host events
│   │   ├── frontend.rs           # frontend selection and release mapping
│   │   ├── lifecycle.rs          # pure startup and presentation policy
│   │   ├── routing.rs            # URL, API ownership, and CORS policy
│   │   ├── splash.rs             # native egui-wgpu startup surface
│   │   ├── webview_handlers.rs   # TurboDoc-specific WebView2 callbacks
│   │   └── host.rs               # winit orchestration and composition root
│   ├── main.rs
│   └── server/
│       ├── api/
...
- **2026-08**: Split the monolithic native application host into focused
  `src/app/` modules for frontend selection, pure lifecycle coordination,
  request and navigation policy, splash rendering, TurboDoc-specific WebView2
  callbacks, and winit orchestration. Move the hosted and proxied URL
  allowlists out of `main.rs` beside their routing tests while retaining the
  existing `FrontendSource` and `run` façade.
